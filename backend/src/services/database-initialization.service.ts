...
export class DatabaseInitializationService {
    ...
    private setInitialAchivements(
        ...
        usersDetails.forEach(user => {
            obs.push(this.dispatcher.dispatch(new CommitEventDto(user.userHash, user.numberOfCommits, 0)));
            obs.push(this.dispatcher.dispatch(new ReviewEventDto(user.userHash, user.finishedReviews, 0)));
        });
