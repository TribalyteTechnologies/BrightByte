...
export abstract class AchievementProcessor {
    ...
    public isAchievementObtained(userHash: string, teamUid: string, version: string): Observable<ResponseDto> {
        return this.userDbSrv.hasAchievement(userHash, teamUid, this.achievement.id, version);
    }
}
